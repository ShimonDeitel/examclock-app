...
struct ContentView: View {
    ...
    @State private var editingItem: ExamclockItem?
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Countdown to every upcoming exam with days-left and prep checklist per test.")
            ...
            HStack {
                Text("\(store.items.count) exams logged")
                    .font(Theme.labelFont())
                    .foregroundStyle(Theme.mutedInk)
                Spacer()
                Text(String(format: "%.1f items", store.totalValue))
                    .font(Theme.labelFont())
                    .foregroundStyle(Theme.accent)
            }
            .padding(.top, 4)
...
struct ExamclockRowView: View {
    let item: ExamclockItem
...
struct AddItemView: View {
    @ObservedObject var store: ExamclockStore
    var editing: ExamclockItem?
    var onComplete: (Bool) -> Void

    @Environment(\.dismiss) private var dismiss
    @State private var title: String = ""
    @State private var category: String = ExamclockCategory.allCases.first?.rawValue ?? ""
    @State private var value: String = ""
    @State private var notes: String = ""

    init(store: ExamclockStore, editing: ExamclockItem? = nil, onComplete: @escaping (Bool) -> Void) {
        self.store = store
        self.editing = editing
        self.onComplete = onComplete
        _title = State(initialValue: editing?.title ?? "")
        _category = State(initialValue: editing?.category ?? (ExamclockCategory.allCases.first?.rawValue ?? ""))
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Value (items)", text: $value)
            ...
            .navigationTitle(editing == nil ? "Add Exam" : "Edit Exam")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
